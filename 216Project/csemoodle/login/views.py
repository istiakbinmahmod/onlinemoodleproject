...
def user_login(request):
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            user_folder = 'static/uploads/'
            if not os.path.exists(user_folder):
                os.mkdir(user_folder)

            img_save_paths = user_folder + 'pro_pic' + str(dbuser) + img_extension
            # img_save_path = user_folder + 'pro_pic'+img_extension
            img_url = 'static/uploads/' + 'pro_pic' + str(dbuser) + img_extension
            print(img_url)
...
def upload_image(request):
    ...
    if request.method == "POST":
        ...
        # password = 'password123'
        sql = "INSERT INTO TAB1(USRNAME, USRIMAGE) VALUES (%s,%s)"
        cursor = connection.cursor()
